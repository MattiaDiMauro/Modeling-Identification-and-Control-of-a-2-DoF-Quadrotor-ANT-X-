...
%% Plot the measured theta
plot(theta.timestamp,theta.value)
hold on
plot(theta0.timestamp,theta0.value)
grid on 
title("Real and reference theta")

%% Rise time
theta_inf=deg2rad(-10);
index_start=0;
index_rise=0;
tol = 1e-6;   

for i = 1:length(theta0.value)
    if abs(theta0.value(i)) > tol
        index_start = i;
        break
    end
end

for i = index_start:length(theta.value)
    if abs(theta.value(i)) - abs(theta_inf) > 0
        index_rise = i;
        break
    end
end

rise_time=theta.timestamp(index_rise)-theta.timestamp(index_start);

%% Settling time
tol_settl_time=deg2rad(2);
index_settl_time=0;
period=2;
for i = index_start:length(theta.value)
    if abs(theta.value(i) - abs(theta_inf))/abs(theta_inf) < tol_settl_time
        index_settl_time = i;
        break
    end
end
settl_time=theta.timestamp(index_settl_time)-theta.timestamp(index_start);
if(settl_time>period)
disp("NO settling time");
end

%% Overshoot
index_overshoot=0;
index_end=0;
for i = index_start:length(theta0.value)
    if abs(theta0.value(i)) < tol
        index_end = i;
        break
    end
end
ymax=max(abs(theta.value(index_start:index_end)));
overshoot=100*(ymax-abs(theta_inf))/abs(theta_inf);

%% Stepinfo
%stepinfo(theta.value(index_start:index_end),theta.timestamp(index_start:index_end))

%% Plot the measured q 
figure
plot(q.timestamp,q.value)
grid on 
title("Real q data")

%% Omega_n and damping estimation
pks_y=[2.0718 1.40726 0.847876 0.316845];
pks_x=[188.691 190.677 192.843 195.081];
diff_vect=[pks_x(2)-pks_x(1) pks_x(3)-pks_x(2) pks_x(4)-pks_x(3)];
average_period=mean(diff_vect);
omega_n=2*pi/average_period;
start_time=188.467;
end_time=197.047;
T_s=end_time-start_time;
damping=4.6/(T_s*omega_n);
...
%% Load identified model data
%par = load("mu_identification.mat");
load("log_2_2024-5-22-17-25-02_2dd.mat");
